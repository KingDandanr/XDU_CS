DATA SEGMENT
    DBuffer DB 30 DUP(0)
    InputInfo DB 0DH, 0AH, '------------------------------------------------', 0DH, 0AH,
            'Please input the number in decimal format:', 0DH, 0AH,
            '> 1. The number must be less than or equal to 65535', 0DH, 0AH,
            '> 2. Enter q or Q to quit', 0DH, 0AH, '> Input number: ', '$'
    ...
    StringInfo DB '------------------------------------------------', 0DH, 0AH,
            'Please input the string to be detect:', 0DH, 0AH, 
            '> Input string: ','$'
...
PUTRESULT:
    ...
    MOV CX, 16
    MOV BL, 0              ; set a flag to avoid the leading zeros
OUTPUT:
    SHL AX, 1             ; the highest bite will be shifted to CF
    JC CF1                ; if CF = 1
